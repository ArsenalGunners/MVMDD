...
For instance, image with name *'bag1_view1_distance1.jpg'* corresponds to the image of *instance #1* of *bag* which captured at *distance1* from *view1*. (Notice that the *instance number* and *view number* are not one-on-one corresponded under two background complexity, the number is used to distinguish it from other instances and views within the current folder. But the distance number is strictly corresponding to the proportion of objects in the picture.)
